    ...
    public class Player : MonoBehaviour
    {
        ...
        v = v != 0 ? v * v / v : 0;//*/
            ...
            if (moveDir != Vector3.zero)
                if (airGravity || jumpHelp)
                    rgb.velocity =
                        new Vector3(0f, rgb.velocity.y,
                            0f); // по y можно добавить множитель для замедленного опускания или наоборот
                else
                    rgb.AddForce(
                        (fix || fixHelp ? fixDir : moveDir) * (grounded ? speed : speed * 0.2f) * Time.deltaTime,
            ...
            currentSpeed.y = 0;
            rgb.AddForce(-currentSpeed * Time.deltaTime * (grounded ? friction : friction * 0.25f),
                ForceMode.VelocityChange);
        ...
        public void FixedUpdate()
        {
            ...
            if (Physics.Raycast(legs.position, moveDir, out hit, 0.5f + 0.5f))
            {
                if (!hit.collider.isTrigger &&
                    hit.collider.tag == "Static") //&&hit.collider.tag=="Static" как возможная проверка
                {
                    ...
                    if (Vector3.Dot(moveDir, hit.normal.normalized) < 0f &&
                        Vector3.Dot(moveDir, hit.normal.normalized) > -0.95f)
            ...
            else if (timerOff)
            {
                ...
                if (moveDir != Vector3.zero)
                {
                    ...
                    {
                        helpDir = new Vector3(obj.transform.position.x - legs.position.x, 0,
                            obj.transform.position.z - legs.position.z);
                        ...
                        if (Physics.Raycast(legs.position, helpDir, out hit, 0.5f + 0.2f))
                        {
                            if (!hit.transform.GetComponent<Collider>().isTrigger && hit.collider.tag == "Static")
                            {
                                ...
                                dirP = fixAngleP * _normal;
                                dirN = fixAngleN * _normal;
                                if (Vector3.Dot(moveDir, hit.normal.normalized) <
                                    0f) // Проверка на напрвление векторов во избежание ненужного обхода
                                    if (Vector3.Dot(moveDir, (dirP).normalized) >
                                        Vector3.Dot(moveDir, (dirN).normalized))
                                    {
                                        sMoveDirH = moveDir;
        ...
        void OnCollisionStay(Collision col)
        {
            ...
            if (Physics.Raycast(legs.position,
                    new Vector3(col.transform.position.x - legs.position.x, 0,
                        col.transform.position.z - legs.position.z), out hit, l))
            {
                if (!hit.transform.GetComponent<Collider>().isTrigger && hit.collider.tag == "Static" &&
                    hit.transform.gameObject == col.gameObject)
                {
                    obj = col.transform;
                }
            }
